...
export const useBubbles = (width, height) => {
	// const { activeBubbleLinkID, bubbleNavLinks } = useContext(SettingAndFilterOptionsContext);
	const { activeBubbleLinkID, colors, ...others } = useSelector((state) => state?.filters);
	const { coins } = useSelector((state) => state?.cryptos);

	const { size: bubbleSort } = bubbleNavLinks?.find(({ size }) => activeBubbleLinkID === size) || {};

	// const { coins, handleActiveBubble } = useContext(DataContext);

	const { createText2, createGradientTexture, createText, createImageSprite } = usePixiUtils();
	...
	const minCircleSize = Math.min(coins?.map((c) => +c.HOUR)) || 50;
	...
	const update = (circles, imageSprites, textSprites, text2Sprites, circleGraphics = []) => {
		...
		return () => {
			for (let i = 0; i < circles.length; i++) {
				...
				const updateCircleChilds = () => {
					circleGraphic.texture = createGradientTexture(circle.radius * 4, circle.color);
				...
				if (true) {
					...
					circle.vx = Math.abs(circle.vx) < minVelocity ? minVelocity * Math.sign(circle.vx) : circle.vx;
					circle.vy = Math.abs(circle.vy) < minVelocity ? minVelocity * Math.sign(circle.vy) : circle.vy;
				} else {
	...
	const clearBubbles = (appContainer, circles, onClearComplete) => {
		// while (stage.children.length > 0) {
		// 	stage.removeChild(stage.children[0]);
		// }
		if (!circles) return;
		// Create a fade-out animation before clearing the bubbles
		circles.forEach((circle, index) => {
			// const container = appContainer.children[0].children[index];
			const container = new PIXI.Container();
			// container.position.set(circle.x, circle.y);
			// container.hitArea = new PIXI.Circle(0, 0, circle.radius);
			// container.eventMode = "dynamic";
			// Animate opacity or scale down
			PIXI.Ticker.shared.add(() => {
				// if (container.alpha > 0) {
				// 	container.alpha -= 0.05; // Adjust the fade-out speed
				// 	container.scale.set(container.scale.x * 0.95, container.scale.y * 0.95); // Shrink effect
				// } else {
				// 	container.destroy();
				// }
				// container.alpha -= 0.005; // Adjust the fade-out speed
				// container.scale.set(container.scale.x * 0.95, container.scale.y * 0.95);
				container.destroy();
			});
		});
		...
		setTimeout(() => {
			...
			onClearComplete && onClearComplete(); // Callback when clearing is complete
		}, 10); // Adjust timeout to match your animation speed
	};
...
//   const handleMouseDown = (event, circle) => {
//     // console.log("called");
//     circle.dragging = true;
...
//   const handleMouseUp = (circle) => {
//     circle.dragging = false;
//     circle.data = null;
//     circle.alpha = 1;
//   };

//   const handleMouseMove = (circle) => {
//     // console.log("called");
//     if (circle.dragging) {
...
//   const handleEmptySpaceClick = (event, circles) => {
//     // console.log("clicked");
...
//       if (distance <= circle.radius) {
//         // console.log(circle);
//         if (!circle.dragging) handleActiveBubble(circle);
//         clickedBubble = true;
//         break;
//       }
//     }
...
//   const addDragEventListeners = (app, circles) => {
...
//       if (selectedCircle && selectedCircle.dragging) {
